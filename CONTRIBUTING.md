...
[Individual Contributor License Agreement](https://github.com/RavindraTirlangi/Klyro/blob/main/klyro/website/docs/legal/contributor-agreement.md).
...
Now you should have a fully functional development environment for the Klyro project. You can start making changes, running tests, and contributing to the project.
...
```
python3 -m venv ../klyro_venv \
 && source ../klyro_venv/bin/activate \
 && pip3 install -e . \
...
Tests clear real provider credentials automatically and use a repository-local
temporary directory. Tests must not depend on a developer's API keys, home
directory contents, running Ollama service, or global Git configuration.
...
```
docker build -t klyro -f docker/Dockerfile .
```
...
2. Navigate to the `klyro/website` directory.
...
The built documentation will be available in the `klyro/website/_site` directory.

## Coding Standards

### Python Compatibility

Klyro supports Python versions 3.10 through 3.14. Pull requests are tested on
all supported versions on Ubuntu and Windows.
...
The project uses [pytest](https://docs.pytest.org/en/latest/) for running unit tests. The test files are located in the `klyro/tests` directory and follow the naming convention `test_*.py`.
...
These workflows are triggered on push and pull request events to the `main` branch, ignoring changes to the `klyro/website/**` and `README.md` files.
...
When contributing new features or making changes to existing code, ensure that you write appropriate tests to maintain code coverage. Follow the existing patterns and naming conventions used in the `klyro/tests` directory.

If you need to mock or create test data, consider adding it to the test files or creating separate fixtures or utility functions within the `klyro/tests` directory.
...
When introducing dependencies, edit only the appropriate
`requirements/*.in` source file. Do not edit generated requirement `.txt`
files manually. Regenerate the complete lock set with:
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\pip-compile.ps1
```

Voice and TUI dependencies belong in their optional-extra input files rather
...
Do not bypass required CI checks. Release tags should be created only after
Ubuntu, Windows, pre-commit, and package smoke-test checks pass.
