...
from klyro.repo import GitRepo
from klyro.utils import GitTemporaryDirectory
...
class TestRepo(unittest.TestCase):
    ...
    @patch("klyro.models.Model.simple_send_with_retries")
    ...
    @patch("klyro.repo.GitRepo.get_commit_message")
    def test_commit_with_custom_committer_name(self, mock_send):
        ...
        with GitTemporaryDirectory():
            ...
            # commit a change with klyro_edits=True (using default attributes)
            fname.write_text("new content")
            commit_result = git_repo.commit(fnames=[str(fname)], klyro_edits=True)
            self.assertIsNotNone(commit_result)

            # check the committer name (defaults interpreted as True)
            commit = raw_repo.head.commit
            self.assertEqual(commit.author.name, "Test User (klyro)")
            self.assertEqual(commit.committer.name, "Test User (klyro)")

            # commit a change without klyro_edits (using default attributes)
            fname.write_text("new content again!")
            commit_result = git_repo.commit(fnames=[str(fname)], klyro_edits=False)
            self.assertIsNotNone(commit_result)
            ...
            commit_result = git_repo_explicit_false.commit(fnames=[str(fname)], klyro_edits=True)
            self.assertIsNotNone(commit_result)
            commit = raw_repo.head.commit
            ...
            commit_result = git_repo_user_no_committer.commit(
                fnames=[str(fname)], klyro_edits=False
            )
            self.assertIsNotNone(commit_result)
            commit = raw_repo.head.commit
            self.assertEqual(
    ...
    def test_commit_with_co_authored_by(self):
        with GitTemporaryDirectory():
            ...
            # commit a change with klyro_edits=True and co-authored-by flag
            fname.write_text("new content")
            commit_result = git_repo.commit(
                fnames=[str(fname)], klyro_edits=True, coder=mock_coder, message="Klyro edit"
            )
            self.assertIsNotNone(commit_result)

            # check the commit message and author/committer
            commit = raw_repo.head.commit
            self.assertIn("Co-authored-by: klyro (gpt-test) <klyro@klyro.chat>", commit.message)
            self.assertEqual(commit.message.splitlines()[0], "Klyro edit")
    ...
    def test_commit_co_authored_by_with_explicit_name_modification(self):
        ...
        with GitTemporaryDirectory():
            ...
            # commit a change with klyro_edits=True and combo flags
            fname.write_text("new content combo")
            commit_result = git_repo.commit(
                fnames=[str(fname)], klyro_edits=True, coder=mock_coder, message="Klyro combo edit"
            )
            self.assertIsNotNone(commit_result)
            ...
            commit = raw_repo.head.commit
            self.assertIn(
                "Co-authored-by: klyro (gpt-test-combo) <klyro@klyro.chat>", commit.message
            )
            self.assertEqual(commit.message.splitlines()[0], "Klyro combo edit")
    ...
    def test_commit_ai_edits_no_coauthor_explicit_false(self):
        # Test AI edits (klyro_edits=True) when co-authored-by is False,
        ...
        with GitTemporaryDirectory():
            ...
            commit_result = git_repo_no_author.commit(
                ...
                message="Klyro no author",
            )
            self.assertIsNotNone(commit_result)
            commit = raw_repo.head.commit
            self.assertNotIn("Co-authored-by:", commit.message)
            self.assertEqual(commit.author.name, "Test User")  # Explicit False
            self.assertEqual(commit.committer.name, "Test User (klyro)")  # Default True
    ...
    def test_get_tracked_files_with_klyroignore(self):
        with GitTemporaryDirectory():
            # new repo
            raw_repo = git.Repo()
            ...
            klyroignore = Path(".klyroignore")
            git_repo = GitRepo(InputOutput(), None, None, str(klyroignore))
            ...
            klyroignore.write_text("new.txt\n")
            time.sleep(2)
            ...
            #
            # klyroignore.write_text("new2.txt\n")
