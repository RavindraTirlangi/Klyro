...
class TestOnboarding(unittest.TestCase):
    ...
    @patch("klyro.onboarding.check_openrouter_tier")
    ...
    @patch("klyro.onboarding.check_openrouter_tier", return_value=True)  # Assume free tier
    ...
    @patch("klyro.onboarding.check_openrouter_tier", return_value=False)  # Assume paid tier
    @patch.dict(os.environ, {"OPENROUTER_API_KEY": "or_key"}, clear=True)
    ...
    @patch("klyro.onboarding.check_openrouter_tier", return_value=False)  # Paid
    @patch.dict(
    ...
    @patch("klyro.onboarding.try_to_select_default_model", return_value="gpt-4o")
    @patch("klyro.onboarding.offer_openrouter_oauth")
    ...
    @patch(
        "klyro.onboarding.try_to_select_default_model", side_effect=[None, None]
    )  # Fails first, fails after oauth attempt
    @patch(
        "klyro.onboarding.offer_openrouter_oauth", return_value=False
    ...
    @patch(
        "klyro.onboarding.try_to_select_default_model",
        side_effect=[None, "openrouter/deepseek/deepseek-r1:free"],
    )  # Fails first, succeeds after oauth
    @patch(
        "klyro.onboarding.offer_openrouter_oauth", return_value=True
    ...
    @patch("klyro.onboarding.start_openrouter_oauth_flow", return_value="new_or_key")
    ...
    @patch("klyro.onboarding.start_openrouter_oauth_flow", return_value=None)  # OAuth fails
    @patch.dict(os.environ, {}, clear=True)
    ...
    @patch("klyro.onboarding.start_openrouter_oauth_flow")
