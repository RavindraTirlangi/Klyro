...
website_files = [
    ...
    "klyro/website/share/index.md",
    "klyro/website/_includes/head_custom.html",
    "klyro/website/_includes/home.css",
    "klyro/website/docs/leaderboards/index.md",
]

exclude_files = [
    "klyro/website/install.ps1",
    "klyro/website/install.sh",
]
...
def blame(start_tag, end_tag=None):
    ...
    files = [
        ...
        or (f.startswith("klyro/resources/") and f.endswith(".yml"))
    ...
    for file in files:
        ...
        if file_counts:
            ...
            for author, count in file_counts.items():
                ...
                if "(klyro)" in author.lower():
                    klyro_total += count

    total_lines = sum(grand_total.values())
    klyro_percentage = (klyro_total / total_lines) * 100 if total_lines > 0 else 0

    end_date = get_tag_date(end_tag if end_tag else "HEAD")

    return all_file_counts, grand_total, total_lines, klyro_total, klyro_percentage, end_date
...
def get_commit_authors(commits):
    ...
    for commit in commits:
        ...
        if lower_subject.startswith("klyro:") or "co-authored-by: klyro" in lower_full:
            author += " (klyro)"
...
def process_all_tags_since(start_tag):
    ...
    for i in tqdm(range(len(tags) - 1), desc="Processing tags"):
        ...
        all_file_counts, grand_total, total_lines, klyro_total, klyro_percentage, end_date = blame(
            start_tag, end_tag
        )
        results.append(
            {
                ...
                "klyro_total": klyro_total,
                "klyro_percentage": round(klyro_percentage, 2),
            }
        )
    return results
...
def main():
    parser = argparse.ArgumentParser(description="Get klyro/non-klyro blame stats")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Find all tags since the specified tag and print klyro percentage between each pair of"
            " successive tags"
        ),
    )
    parser.add_argument(
    ...
    if not args.all_since:
        print(f"- Klyro wrote {round(klyro_percentage)}% of the code in this release.")
