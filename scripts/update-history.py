...
def get_latest_version_from_history():
    ...
    match = re.search(r"### Klyro v(\d+\.\d+\.\d+)", history_content)
    if not match:
...
def main():
    ...
    version_header = f"### Klyro v{latest_ver}"
    ...
    next_version_idx = history_content.find("\n### Klyro v", version_idx + len(version_header))
    ...
    # Run blame to get klyro percentage
    blame_result = subprocess.run(["python3", "scripts/blame.py"], capture_output=True, text=True)
    klyro_line = blame_result.stdout.strip().split("\n")[-1]  # Get last line with percentage

    # Construct and run the klyro command
    message = history_prompt.format(klyro_line=klyro_line)
    ...
    # Run update-docs.sh after klyro
