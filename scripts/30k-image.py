...
# flake8: noqa: E501
"""
Generate a celebratory SVG image for Klyro reaching 30,000 GitHub stars.
...
def embed_font():
    ...
    font_path = (
        Path(__file__).parent.parent / "klyro" / "website" / "assets" / "Glass_TTY_VT220.ttf"
    )
...
def generate_confetti(count=150, width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT):
    ...
    confetti = []
    colors = [KLYRO_GREEN, KLYRO_BLUE, GOLD_COLOR, "#e74c3c", "#9b59b6", "#3498db", "#2ecc71"]
...
<svg width="{width}" height="{height}" viewBox="0 0 {width} {height}" xmlns="http://www.w3.org/2000/svg">
  ...
  <style>
    ...
    .klyro-logo {{ font-family: 'GlassTTYVT220', monospace; font-size: 120px; fill: {KLYRO_GREEN}; text-anchor: middle; filter: url(#glow); }}
  ...
  <g clip-path="url(#rounded-rect)">
    ...
    <!-- Main content -->
    <text x="{width/2}" y="{height/2 - 100}" class="klyro-logo">klyro</text>
    ...
    <text x="{width/2}" y="{height - 50}" class="footer">github.com/RavindraTirlangi/Klyro</text>
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate a celebration SVG for Klyro's 30K GitHub stars"
    )
    parser.add_argument(
        "--output",
        "-o",
        type=str,
        default="klyro-30k-stars.svg",
        help="Output file path (default: klyro-30k-stars.svg)",
    )
    parser.add_argument(
        "--width",
        "-w",
