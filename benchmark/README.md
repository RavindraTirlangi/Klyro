...
# Klyro benchmark harness

Klyro uses benchmarks to quantitatively measure how well it works
with various LLMs.
...
This
benchmark evaluates how effectively klyro and LLMs can translate a
...
See [this writeup for a longer discussion about the benchmark](https://klyro.chat/2024/12/21/polyglot.html).
...
There are 3 main tasks involved in benchmarking klyro:
...
git clone https://github.com/Klyro-AI/polyglot-benchmark tmp.benchmarks/polyglot-benchmark
...
# Inside the container, install klyro as a development build.
...
- `--model` is the name of the model, same as you would pass directly to `klyro`.
- `--edit-format` is the name of the edit format, same as you would pass directly to `klyro`. When working with an experimental LLM, I recommend starting with `whole`
...
- `--read-model-settings=<filename.yml>` specify model settings, see here: https://klyro.chat/docs/config/adv-model-settings.html#model-settings
