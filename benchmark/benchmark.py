...
from klyro import models, sendchat
from klyro.coders import Coder, base_coder
from klyro.dump import dump  # noqa: F401
from klyro.io import InputOutput

BENCHMARK_DNAME = Path(os.environ.get("KLYRO_BENCHMARK_DIR", "tmp.benchmarks"))
...
def main(
    ...
    replay: str = typer.Option(
        None,
        "--replay",
        help="Replay previous .klyro.chat.history.md responses from previous benchmark run",
    ),
    ...
    no_klyro: bool = typer.Option(False, "--no-klyro", help="Do not run klyro"),
    ...
    read_model_settings: str = typer.Option(
        None, "--read-model-settings", help="Load klyro model settings from YAML file"
    ),
    ...
    if "KLYRO_DOCKER" not in os.environ:
...
def show_diffs(dirnames):
    ...
    for testcase in testcases:
        ...
        for outcome, dirname in zip(all_outcomes, dirnames):
            print(outcome, f"{dirname}/{testcase}/.klyro.chat.history.md")
...
def load_results(dirname, stats_languages=None):
    ...
    if stats_languages:
        ...
        glob_patterns = [f"{lang}/exercises/practice/*/.klyro.results.json" for lang in languages]
    else:
        glob_patterns = ["*/exercises/practice/*/.klyro.results.json"]
    ...
    for hsh in commit_hashes:
        ...
        try:
            version = subprocess.check_output(
                ["git", "show", f"{hsh}:klyro/__init__.py"], universal_newlines=True
            )
...
def get_replayed_content(replay_dname, test_dname):
    ...
    replay_fname = replay_dname / test_name / ".klyro.chat.history.md"
    dump(replay_fname)
...
def run_test(original_dname, testdir, *args, **kwargs):
    try:
        ...
        results_fname = testdir / ".klyro.results.json"
...
def run_test_real(
    ...
    history_fname = testdir / ".klyro.chat.history.md"
...
def run_unit_tests(original_dname, testdir, history_fname, test_files):
    ...
    TEST_COMMANDS = {
        ...
        ".go": ["go", "test", "./..."],
        ".js": ["/klyro/benchmark/npm-test.sh"],
        ".cpp": ["/klyro/benchmark/cpp-test.sh"],
        ".java": ["./gradlew", "test"],
    }
