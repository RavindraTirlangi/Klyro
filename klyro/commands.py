...
from klyro.utils import is_image_file
...
class Commands:
    ...
    def cmd_chat_mode(self, args):
        ...
        show_formats = OrderedDict(
            [
                ("help", "Get help about using klyro (usage, config, troubleshoot)."),
    ...
    def cmd_export(self, args):
        ...
        filename = args.strip() if args.strip() else "klyro-export.md"

        # Read from the chat history file if it exists
        chat_history_file = getattr(self.io, "chat_history_file", None)
        if chat_history_file and Path(chat_history_file).exists():
            content = Path(chat_history_file).read_text(encoding="utf-8", errors="ignore")
        else:
            # Fall back to building from done_messages + cur_messages
            parts = []
            for msg in self.coder.done_messages + self.coder.cur_messages:
        ...
        self.io.tool_output(f"Chat history exported to {export_path}")
    ...
    def cmd_undo(self, args):
        "Undo the last git commit if it was done by klyro"
        try:
    ...
    def raw_cmd_undo(self, args):
        ...
        if last_commit_hash not in self.coder.klyro_commit_hashes:
            self.io.tool_error("The last commit was not made by klyro in this chat session.")
            self.io.tool_output(
    ...
    def cmd_add(self, args):
        ...
        for word in filenames:
            ...
            if self.coder.repo and self.coder.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} due to klyroignore or --subtree-only.")
                continue
    ...
    def basic_help(self):
        ...
        self.io.tool_output()
        self.io.tool_output("Use `/help <question>` to ask questions about how to use klyro.")

    def cmd_help(self, args):
        "Ask questions about klyro"
        ...
        user_msg += """
# Announcement lines from when this session of klyro was launched:
...
"""
    ...
    def cmd_report(self, args):
        ...
        from klyro.report import report_github_issue
