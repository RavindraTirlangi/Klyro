...
from klyro import __version__, models, prompts, urls, utils
from klyro.analytics import Analytics
from klyro.commands import Commands
from klyro.exceptions import LiteLLMExceptions
from klyro.history import ChatSummary
from klyro.io import ConfirmGroup, InputOutput
from klyro.linter import Linter
from klyro.llm import litellm
from klyro.models import RETRY_TIMEOUT
from klyro.reasoning_tags import (
    REASONING_TAG,
...
from klyro.repomap import RepoMap
from klyro.run_cmd import run_cmd
from klyro.utils import format_content, format_messages, format_tokens, is_image_file
from klyro.waiting import WaitingSpinner
...
class Coder:
    ...
    repo = None
    last_klyro_commit_hash = None
    klyro_edited_files = None
        ...
        **kwargs,
        ...
        if from_coder:
            ...
            update = dict(
                ...
                klyro_commit_hashes=from_coder.klyro_commit_hashes,
    ...
    def __init__(
        ...
        if klyro_commit_hashes:
            self.klyro_commit_hashes = klyro_commit_hashes
        else:
            self.klyro_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches klyroignore spec.")
                continue
    ...
    def warm_cache(self, chunks):
        ...
        delay = 5 * 60 - 5
        delay = float(os.environ.get("KLYRO_CACHE_KEEPALIVE_DELAY", delay))
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.klyro_edited_files.update(edited)
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(fnames=edited, context=context, klyro_edits=True, coder=self)
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_klyro_commit_hash = commit_hash
        self.klyro_commit_hashes.add(commit_hash)
        self.last_klyro_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each klyro commit.")
