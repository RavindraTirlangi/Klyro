...
from klyro.mdstream import MarkdownStream
...
# Constants
NOTIFICATION_MESSAGE = "Klyro is waiting for your input"
...
class InputOutput:
    ...
    def __init__(
        ...
        self.append_chat_history(f"\n# klyro chat started at {current_time}\n\n")
    ...
    def get_default_notification_command(self):
        ...
        if system == "Darwin":  # macOS
            ...
            if shutil.which("terminal-notifier"):
                return f"terminal-notifier -title 'Klyro' -message '{NOTIFICATION_MESSAGE}'"
            # Fall back to osascript
            return (
                f'osascript -e \'display notification "{NOTIFICATION_MESSAGE}" with title "Klyro"\''
            )
        elif system == "Linux":
            ...
            for cmd in ["notify-send", "zenity"]:
                if shutil.which(cmd):
                    if cmd == "notify-send":
                        return f"notify-send 'Klyro' '{NOTIFICATION_MESSAGE}'"
                    elif cmd == "zenity":
