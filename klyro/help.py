...
from klyro import __version__, utils
from klyro.dump import dump  # noqa: F401
from klyro.help_pats import exclude_website_pats
...
def get_package_files():
    for path in importlib_resources.files("klyro.website").iterdir():
        if path.is_file():
            yield path
        elif path.is_dir():
...
def fname_to_url(filepath):
    ...
    return f"https://klyro.chat/{url_path}"
...
def get_index():
    ...
    dname = Path.home() / ".klyro" / "caches" / ("help." + __version__)
    ...
    if index is None:
        ...
        for fname in get_package_files():
            ...
            doc = Document(
                text=importlib_resources.files("klyro.website")
                .joinpath(fname)
