...
from klyro.copypaste import ClipboardWatcher
from klyro.deprecated import handle_deprecated_model_args
from klyro.format_settings import format_settings, scrub_sensitive_info
from klyro.history import ChatSummary
from klyro.io import InputOutput
from klyro.llm import litellm  # noqa: F401; properly init litellm on launch
from klyro.models import ModelSettings
from klyro.onboarding import offer_openrouter_oauth, select_default_model
from klyro.repo import ANY_GIT_ERROR, GitRepo
from klyro.report import report_uncaught_exceptions
from klyro.versioncheck import check_version, install_from_main_branch, install_upgrade
...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning(
            "You should probably run klyro in your project's directory, not your home dir."
        )
        return
    elif cwd and io.confirm_ask(
        "No git repo found, create one to track klyro's changes (recommended)?"
    ):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        ...
        if not repo.ignored(".klyro"):
            patterns_to_add.append(".klyro*")
...
def check_streamlit_install(io):
    return utils.check_pip_install_extra(
        io,
        "streamlit",
        "You need to install the klyro browser feature",
        ["klyro[browser]"],
    )
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        ".klyro.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading klyro model settings: {e}")
        return 1
...
def load_dotenv_files(git_root, dotenv_fname, encoding="utf-8"):
    ...
    oauth_keys_file = Path.home() / ".klyro" / "oauth-keys.env"
...
def register_litellm_models(git_root, model_metadata_fname, io, verbose=False):
    ...
    resource_metadata = importlib_resources.files("klyro.resources").joinpath("model-metadata.json")
    model_metadata_files.append(str(resource_metadata))

    model_metadata_files += generate_search_path_list(
        ".klyro.model.metadata.json", git_root, model_metadata_fname
    )
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        io.tool_error("Klyro only works with git repos with version number 1 or 2.")
        io.tool_output("You may be able to convert your repo: git update-index --index-version=2")
        io.tool_output("Or run klyro --no-git to proceed without using git.")
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    conf_fname = Path(".klyro.conf.yml")
    ...
    if args.dark_mode:
        ...
        args.assistant_output_color = "white"
    ...
    if args.light_mode:
        ...
        args.assistant_output_color = "black"
    ...
    if args.analytics is not False:
        if analytics.need_to_ask(args.analytics):
            io.tool_output(
                "Klyro respects your privacy and never collects your code, chat messages, keys or"
                " personal info."
            )
            io.tool_output(f"For more info: {urls.analytics}")
            disable = not io.confirm_ask(
                "Allow collection of anonymous analytics to help improve klyro?"
            )
    ...
    if args.klyroignore:
        ignores.append(args.klyroignore)
    ...
    if args.apply:
        ...
        # For testing #2879
        # from klyro.coders.base_coder import all_fences
...
def is_first_run_of_new_version(io, verbose=False):
    ...
    installs_file = Path.home() / ".klyro" / "installs.json"
    ...
    try:
        if is_first_run:
            ...
            try:
                ...
                io.tool_error(str(err))
                io.tool_output("Error loading required imports. Did you install klyro properly?")
