...
from klyro import prompts, utils
...
class GitRepo:
    repo = None
    klyro_ignore_file = None
    klyro_ignore_spec = None
    klyro_ignore_ts = 0
    klyro_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if klyro_ignore_file:
            self.klyro_ignore_file = Path(klyro_ignore_file)

    def commit(self, fnames=None, context=None, message=None, klyro_edits=False, coder=None):
        """
        ...
        Args:
            ...
            klyro_edits (bool, optional): Whether the changes were made by Klyro. Defaults to False.
        ...
        Klyro generated the changes (`klyro_edits`).
        ...
        - klyro_edits=True: Changes were generated by Klyro (LLM).
        - klyro_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (klyro)".
        - --attribute-committer: Modify Committer name to "User Name (klyro)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: klyro (<model>) <klyro@klyro.chat>" trailer to commit message.

        Behavior Summary:

        1. When klyro_edits = True (AI Changes):
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(klyro), Committer=You, Trailer added
        - User commit (defaults): klyro_edits=False -> Author=You, Committer=You(klyro)
        - User commit with explicit no-committer: klyro_edits=False,
        ...
        prefix_commit_message = klyro_edits and (
        ...
        if klyro_edits and attribute_co_authored_by:
            model_name = "unknown-model"
            if coder and hasattr(coder, "main_model") and coder.main_model.name:
                model_name = coder.main_model.name
            commit_message_trailer = f"\n\nCo-authored-by: klyro ({model_name}) <klyro@klyro.chat>"

        # Determine if author/committer names should be modified
        # Author modification applies only to klyro edits.
        # It's used if effective_author is True AND
        # (co-authored-by is False OR author was explicitly set).
        use_attribute_author = (
            klyro_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of klyro_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an klyro edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (klyro_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        if prefix_commit_message:
            commit_message = "klyro: " + commit_message
        ...
        committer_name = f"{original_user_name} (klyro)"
    ...
    def refresh_klyro_ignore(self):
        if not self.klyro_ignore_file:
            return

        current_time = time.time()
        if current_time - self.klyro_ignore_last_check < 1:
            return

        self.klyro_ignore_last_check = current_time

        if not self.klyro_ignore_file.is_file():
            return

        mtime = self.klyro_ignore_file.stat().st_mtime
        if mtime != self.klyro_ignore_ts:
            self.klyro_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.klyro_ignore_file.read_text().splitlines()
            self.klyro_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        if not self.klyro_ignore_file or not self.klyro_ignore_file.is_file():
            return False
        ...
        return self.klyro_ignore_spec.match_file(fname)
