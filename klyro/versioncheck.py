...
import klyro
from klyro import utils
from klyro.dump import dump  # noqa: F401

VERSION_CHECK_FNAME = Path.home() / ".klyro" / "caches" / "versioncheck"


def install_from_main_branch(io):
    """
    Install the latest version of klyro from the main branch of the GitHub repository.
    """
...
def install_upgrade(io, latest_version=None):
    """
    Install the latest version of klyro from PyPI.
    """

    if latest_version:
        new_ver_text = f"Newer klyro version v{latest_version} is available."
    else:
        new_ver_text = "Install latest version of klyro?"

    docker_image = os.environ.get("KLYRO_DOCKER_IMAGE")
    if docker_image:
        text = f"""
...
"""
    ...
    if success:
        io.tool_output("Re-run klyro to use new version.")
        sys.exit()
...
def check_version(io, just_check=False, verbose=False):
    ...
    try:
        response = requests.get("https://pypi.org/pypi/klyro/json", timeout=10)
        data = response.json()
