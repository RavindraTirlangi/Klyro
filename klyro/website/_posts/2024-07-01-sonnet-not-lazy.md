...
[![sonnet is the opposite of lazy](/assets/sonnet-not-lazy.jpg)](https://klyro.chat/assets/sonnet-not-lazy.jpg)
...
Sonnet's score on
[klyro's refactoring benchmark](https://klyro.chat/docs/leaderboards/#code-refactoring-leaderboard)
...
[klyro's latest release](https://klyro.chat/HISTORY.html#klyro-v0410)
were thrilled:

- *Works like a charm. It is a monster. It refactors files of any size like it is nothing. The continue trick with Sonnet is truly the holy grail. Klyro beats [other tools] hands down. I'm going to cancel both subscriptions.* -- [Emasoft](https://github.com/RavindraTirlangi/Klyro/issues/705#issuecomment-2200338971)
- *Thanks heaps for this feature - it's a real game changer. I can be more ambitious when asking Claude for larger features.* -- [cngarrison](https://github.com/RavindraTirlangi/Klyro/issues/705#issuecomment-2196026656)
- *Fantastic...! It's such an improvement not being constrained by output token length issues. [I refactored] a single JavaScript file into seven smaller files using a single Klyro request.* -- [John Galt](https://discord.com/channels/1131200896827654144/1253492379336441907/1256250487934554143)
...
Klyro now allows Sonnet to return code in multiple 4k token
responses.
Klyro seamlessly combines them so that Sonnet can return arbitrarily
long responses.
...
Klyro now prompts Sonnet to discourage these long-winded
...
[The latest release of klyro](https://klyro.chat/HISTORY.html#klyro-v0410)
has specialized support for Claude 3.5 Sonnet:

- Klyro allows Sonnet to produce as much code as it wants,
by automatically and seamlessly spreading the response
out over a sequence of 4k token API responses.
- Klyro carefully prompts Sonnet to be concise when proposing
code edits.
This reduces Sonnet's tendency to waste time, tokens and money
returning large chunks of unchanging code.
- Klyro now uses Claude 3.5 Sonnet by default if the `ANTHROPIC_API_KEY` is set in the environment.

See 
[klyro's install instructions](https://klyro.chat/docs/install.html)
