...
excerpt: "Benchmark results for Qwen3 models using the Klyro polyglot coding benchmark."
highlight_image: /assets/2025-05-08-qwen3.jpg
date: 2025-05-08
---

# Qwen3 results on the klyro polyglot benchmark
...
Below are collection of klyro polyglot benchmark results for the new Qwen3 models.
Results are presented using both "diff" and "whole" 
[edit formats](https://klyro.chat/docs/more/edit-formats.html),
with various models settings, against various API providers.

See details on the 
[model settings](https://klyro.chat/docs/config/adv-model-settings.html#model-settings) 
used after the results table.

{: .note }
This article is being updated as new results become available.
Also, some results were submitted by klyro users and have not been verified.

<h2 id="leaderboard-title">Qwen3 results on the klyro polyglot benchmark</h2>
...
<script>
const LEADERBOARD_CUSTOM_TITLE = "Qwen3 results on the klyro polyglot benchmark";
...
non-thinking model settings in `.klyro.model.settings.yml`:
...
And then running klyro:

```bash
klyro --model openrouter/qwen/qwen3-235b-a22b
```


## OpenRouter, all providers, default settings (thinking)

These results were obtained by simply running klyro as shown below, without any model specific settings.
This should have enabled thinking, assuming upstream API providers honor that convention for Qwen3.

```bash
klyro --model openrouter/qwen/qwen3-xxx
```

## VLLM, bfloat16, recommended /no_think

These [benchmarks results were obtained by GitHub user AlongWY](https://github.com/RavindraTirlangi/Klyro/pull/3908)
with the 
[recommended](https://huggingface.co/Qwen/Qwen3-235B-A22B#best-practices)
non-thinking model settings in `.klyro.model.settings.yml`:
...
```bash
klyro --model openai/<model-name> --openai-api-base <url>
```
