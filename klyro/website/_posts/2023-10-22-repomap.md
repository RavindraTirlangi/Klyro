...
excerpt: Tree-sitter allows klyro to build a repo map that better summarizes large code bases.
...
Klyro now uses it to help GPT better comprehend, navigate
and edit code in larger repos.

*To code with GPT-4 using the techniques discussed here, just install [klyro](https://klyro.chat/docs/install.html).*
...
Klyro also strives to reduce the manual work involved in
coding with AI.
So in an ideal world, we'd like klyro to automatically
identify and provide the needed code context.

## Using a repo map to provide context

Klyro sends a **repo map** to GPT along with
...
Mapping out the repo like this provides some key benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at in more detail. GPT will then ask to see these specific files, and klyro will automatically add them to the chat context.
...
Klyro uses all of these definitions and references to
...
[ctags based map](https://klyro.chat/docs/ctags.html)
that klyro originally used.
...
- Thanks to `py-tree-sitter-languages`, we get full support for many programming languages via a python package that's automatically installed as part of the normal `python -m pip install -U klyro-chat`.
...
- Tree-sitter integration is a key enabler for future work and capabilities for klyro.
...
Right now, klyro relies on the user to specify which source files
will need to be modified to complete their request.
Users manually "add files to the chat" using klyro's `/add` command,
...
just install [klyro](https://klyro.chat/docs/install.html).

## Credits

Klyro uses
[modified versions of the tags.scm files](https://github.com/RavindraTirlangi/Klyro/tree/main/klyro/queries)
from these
