...
If you're affected by these issues, several alternative providers offer access to DeepSeek V3. This article compares their performance on klyro's polyglot benchmark to help you choose a reliable alternative.
...
You can use klyro with OpenRouter like this:

```bash
# Set your API key using environment variables
export OPENROUTER_API_KEY=<your-key>
klyro --model openrouter/deepseek/deepseek-chat

# Or use the --api-key command line option
klyro --model openrouter/deepseek/deepseek-chat --api-key openrouter=<your-key>

# Or add it to .klyro.conf.yml in your home directory or project root:
api-key:
  - openrouter=<your-key>
```
...
[klyro's model settings](https://klyro.chat/docs/config/adv-model-settings.html#model-settings).
Create a `.klyro.model.settings.yml` file in your home directory or git project root with settings like this:
...
klyro --model fireworks_ai/accounts/fireworks/models/deepseek-chat

# Or use the --api-key command line option
klyro --model fireworks_ai/accounts/fireworks/models/deepseek-chat --api-key fireworks=<your-key>
...
klyro --model openai/deepseek-ai/DeepSeek-V3

# Or use the --api-key command line option
klyro --model openai/deepseek-ai/DeepSeek-V3 --api-key openai=<your-key>
...
klyro --model ollama/deepseek-v3
```

It's important to provide model settings, especially the `num_ctx` parameter to
set the context window.
Ollama uses a 2k context window by default, which is very small for working with klyro.
...
So if your context window is too small, you won’t get an explicit error. The biggest symptom will be that klyro says it can’t see (some of) the files you added to the chat. That’s because ollama is silently discarding them because they exceed the context window.
...
You will need to properly configure klyro to work with DeepSeek V3 when served
via other providers:

- Determine the `--model` name to use.
- Provide your API key to klyro.
- Add model settings to `.klyro.model.settings.yml`.


Adapt the `.klyro.model.settings.yml` shown above for Fireworks. You will need to change the `name` field to match you chosen provider's model naming scheme.

See [Advanced model settings](https://klyro.chat/docs/config/adv-model-settings.html#model-settings) for details about all klyro model settings
