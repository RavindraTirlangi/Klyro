...
Klyro now has experimental support for using two models to complete each coding task:
...
[klyro's code editing benchmark](/docs/benchmarks.html#the-benchmark).
...
This approach has recently become attractive for klyro due to 
...
incompatible with klyro's goal of providing an interactive,
pair programming AI coding experience.

## Code reasoning and code editing

Normally klyro asks the model to solve a coding problem in one prompt,
asking the LLM to explain the solution and return 
a well formatted series of file edits.
All of [klyro's editing formats](/docs/more/edit-formats.html)
require the LLM to return source code edits in a specific text
format, so that klyro can process the edits and apply them to the local source files.
...
The development version of klyro 
...
klyro --sonnet --architect

# Work with OpenAI models, using gpt-4o as the Editor
export OPENAI_API_KEY=your-key-goes-here
klyro --4o --architect
klyro --o1-mini --architect
klyro --o1-preview --architect
```

## More info

Klyro has a number of "chat modes", and "architect" is available as a new chat mode.
The `--architect` switch is a shortcut for `--chat-mode architect`.
For more details, see documentation on 
[klyro's chat modes](/docs/usage/modes.html).
...
by itself in klyro's normal "code" editing mode
