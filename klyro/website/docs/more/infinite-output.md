...
parent: More info
nav_order: 480
description: Klyro can handle "infinite output" from models that support prefill.
---
...
Klyro is able to work around this limit with models that support
"prefilling" the assistant response.
When you use klyro with a model that supports prefill, you will see
...
When klyro is collecting code edits from a model and
it hits the output token limit,
klyro simply initiates another LLM request with the partial
response prefilled.
...
Klyro supports "infinite output" for models that support "prefill",
such as:
