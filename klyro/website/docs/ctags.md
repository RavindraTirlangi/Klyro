...
Klyro no longer uses ctags to build a repo map.
Please see the newer article about
[using tree-sitter to build a better repo map](https://klyro.chat/docs/repomap.html).
...
To address these issues, `klyro` now
...
[chat transcript](https://klyro.chat/examples/add-test.html)
...
To code with GPT-4 using the techniques discussed here:
  ...
  - Install [klyro](https://klyro.chat/docs/install.html).
  - Install universal ctags.
  - Run `klyro` inside your repo, and it should say "Repo-map: universal-ctags using 1024 tokens".
...
This works pretty well, and is supported by `klyro` -- you
...
The latest version of `klyro` sends a **repo map** to GPT along with
...
Mapping out the repo like this provides some benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at. GPT will then ask to see these specific files, and `klyro` will automatically add them to the chat context (with user approval).
...
Sending an appropriate subset of the repo map would help `klyro` work
