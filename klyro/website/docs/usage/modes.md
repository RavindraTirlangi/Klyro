...
Klyro has a few different chat modes:

- `code` - Klyro will make changes to your code to satisfy your requests.
- `ask` - Klyro will discuss your code and answer questions about it, but never make changes.
- `architect` - Like code mode, klyro will change your files. An architect model will propose changes and an editor model will translate that proposal into specific file edits.
- `help` - Klyro will answer questions about klyro: usage, configuration, troubleshooting, etc.

By default, klyro starts in "code" mode. As you are talking, you can
...
Or you can launch klyro in one of the modes with the `--chat-mode <mode>` switch.
There is also a special shortcut `--architect` to launch in `--chat-mode architect`.

The klyro prompt will indicate the active mode:
...
Use ask mode to discuss what you want to do, get suggestions or options from klyro
and provide feedback on the approach.
Once klyro understands the mission, switch to code mode to have it start editing
your files.
...
have klyro execute on the plan you've been discussing.
...
When you are in architect mode, klyro sends your requests to two models:
...
2. Klyro then sends another request to an "editor model",
asking it to turn the architect's proposal into specific file editing instructions.
Klyro has built in defaults to select an editor model based on your main model.
...
The editor model uses one of klyro's edit formats to let the LLM
edit source files.
Klyro will pick a suitable edit format by default,
...
See this article on 
[klyro's architect/editor mode](/2024/09/26/architect.html)
for more details.
...
#### /help How do I use klyro with ollama?

Run `klyro --model ollama/<ollama-model>`.
See these docs for more info: https://klyro.chat/docs/llms/ollama.html
