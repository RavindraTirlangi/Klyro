...
parent: Usage
nav_order: 750
description: Klyro supports prompt caching for cost savings and faster coding.
---

# Prompt caching

Klyro supports prompt caching for cost savings and faster coding.
Currently Anthropic provides caching for Sonnet and Haiku,
and DeepSeek provides caching for Chat.

Klyro organizes the chat history to try and cache:
...
Run klyro with `--cache-prompts` or add that setting to your 
...
When caching is enabled, it will be noted for the main model when klyro launches:
...
Klyro can ping the provider to keep your prompt cache warm and prevent
it from expiring.
By default, Anthropic keeps your cache for 5 minutes.
Use `--cache-keepalive-pings N` to tell klyro to ping
every 5 minutes to keep the cache warm.
Klyro will ping up to `N` times over a period of `N*5` minutes
