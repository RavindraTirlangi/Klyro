...
parent: Usage
nav_order: 850
description: Klyro works with LLM web chat UIs
---
...
<div class="video-container">
  <video controls loop poster="/assets/copypaste.jpg">
    ...
    <a href="/assets/copypaste.mp4">Klyro browser UI demo video</a>
  </video>
</div>
...
[Klyro can connect to most LLMs via API](https://klyro.chat/docs/llms.html) and works best that way.
...
Klyro has features for working with an LLM via its web chat interface.
This allows you to use the web chat LLM as the "big brain code architect"
while running klyro with a smaller, cheaper LLM to actually make changes
to your local files.

For this "file editor" part of the process 
you can run klyro with many open source, free or very inexpensive LLMs.
For example, the demo video above shows klyro using DeepSeek to apply the changes
that o1-preview is suggesting in the web chat.

### Copy klyro's code context to your clipboard, paste into the web UI

The `/copy-context <instructions>` command can be used in chat to copy klyro's code context to your clipboard.
It will include:

- All the files which have been added to the chat via `/add`.
- Any read only files which have been added via `/read`.
- Klyro's [repository map](https://klyro.chat/docs/repomap.html) that brings in code context related to the above files from elsewhere in your git repo.
...
### Paste the LLM's reply back into klyro to edit your files

Once the LLM has replied, you can use the "copy response" button in the web UI to copy
the LLM's response.
Back in klyro, you can run `/paste` and klyro will edit your files
...
This works best if you run klyro with `--edit-format editor-diff` or `--edit-format editor-whole`.

### Copy/paste mode

Klyro has a `--copy-paste` mode that streamlines this entire process:

- Whenever you `/add` or `/read` files, klyro will automatically copy the entire, updated
code context to your clipboard. 
You'll see "Copied code context to clipboard" whenever this happens.
- When you copy the LLM reply to your clipboard outside klyro, klyro will automatically notice
and load it into the klyro chat. 
Just press ENTER to send the message
and klyro will apply the LLMs changes to your local files.
- Klyro will automatically select the best edit format for this copy/paste functionality. 
Depending on the LLM you have klyro use, it will be either `editor-whole` or `editor-diff`.
...
Klyro's web chat features have been designed to be compliant with the 
terms of service of most LLM web chats.

There are 4 copy/paste steps involved when coding with an LLM web chat:

1. Copy code and context from klyro.
2. Paste the code and context into the LLM web chat.
3. Copy the reply from the LLM web chat.
4. Paste the LLM reply into klyro.

Most LLM web chat TOS prohibit automating steps (2) and (3) where code
is copied from and pasted into the web chat.
Klyro's `--copy-paste` mode leaves those as 100% manual steps for the user to complete.
It simply streamlines steps (1) and (4) that are interactions with klyro,
and which should not be under the scope of an LLM web chat TOS.

If you are concerned that
the automatic interactions with klyro in steps (1) and (4) may be problematic with respect to
