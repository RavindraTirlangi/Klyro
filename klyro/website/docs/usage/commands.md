...
parent: Usage
nav_order: 50
description: Control klyro with in-chat commands like /add, /model, etc.
---
...
Klyro supports commands from within the chat, which all start with `/`.

<!--[[[cog
from klyro.commands import get_help_md
cog.out(get_help_md())
]]]-->

|Command|Description|
|:------|:----------|
| **/add** | Add files to the chat so klyro can edit them or review them in detail |
...
| **/help** | Ask questions about klyro |
...
| **/undo** | Undo the last git commit if it was done by klyro |
...
It's always safe to use Control-C to interrupt klyro if it isn't providing a useful response. The partial response remains in the conversation, so you can refer to it when you reply to the LLM with more information or direction.
...
To use vi/vim keybindings, run klyro with the `--vim` switch.
