...
parent: Usage
nav_order: 800
description: Tell klyro to follow your coding conventions when it works on your code.
---
...
The easiest way to do that with klyro is to simply create
...
and then we can add it to the klyro chat, along with the file(s)
that we want to edit.

It's best to load the conventions file with `/read CONVENTIONS.md` 
or `klyro --read CONVENTIONS.md`. 
This way it is marked as read-only, and cached if prompt caching
is enabled.

## Community contributed conventions

You can check the [klyro conventions repository](https://github.com/Klyro-AI/conventions)
to find or contribute conventions files.

## Always load conventions

You can also configure klyro to always load your conventions file
in the [`.klyro.conf.yml` config file](https://klyro.chat/docs/config/klyro_conf.html):
...
> $ klyro --read CONVENTIONS.md useragent.py  
>  
> Klyro v0.24.2-dev  
