---
title: Klyro in your IDE
#highlight_image: /assets/browser.jpg
parent: Usage
nav_order: 750
description: Klyro can watch your files and respond to AI comments you add in your favorite IDE or text editor.
---
...
<div class="video-container">
  <video controls loop poster="/assets/watch.jpg">
    ...
    <a href="/assets/watch.mp4">Klyro browser UI demo video</a>
  </video>
</div>
...
If you run klyro with `--watch-files`, it will watch all files in your repo
and look for any AI coding instructions you add using your favorite IDE or text editor.

Specifically, klyro looks for one-liner comments (# ... or // ...) that either start or end with `AI`, `AI!` or `AI?` like these:
...
Klyro will take note of all the comments that start or end with `AI`.
Comments that include `AI!` with an exclamation point or `AI?` with a question
mark are special.
They trigger klyro to take action to collect *all* the AI comments and use them
as your instructions.

- `AI!` triggers klyro to make changes to your code.
- `AI?` triggers klyro to answer your question.

See the demo video above that shows klyro working with AI comments in VSCode.
...
Then klyro would update the file and implement the function:
...
Klyro only watches for these types of **one-liner** comments:
...
Klyro will look for those comment types in all files.
...
before triggering klyro with a final `AI!`.
...
Just use `AI!` last, to trigger klyro.
...
klyro's attention to the block.
...
### Add a file to the klyro chat

Rather than using `/add` to add a file inside the klyro chat, you can
simply put an `#AI` comment in it and save the file.
You can undo/remove the comment immediately if you like, the file
will still be added to the klyro chat.

## Also use klyro chat in the terminal
...
but it can sometimes be effective to switch over to the klyro terminal chat.
The chat has the history of the AI comments you just made,
so you can continue on naturally from there.

You can also use the normal klyro chat in your terminal to work with
many of klyro's more advanced features:
...
- Use [chat modes](https://klyro.chat/docs/usage/modes.html) to ask questions or get help.
...
- [Fix lint and test errors](https://klyro.chat/docs/usage/lint-test.html).
- Run shell commands.
- Etc.
...
you can let klyro infer more about the request.
...
As you use klyro with your chosen LLM, you can develop a sense for how
explicit you need to make your AI comments.

## Behind the scenes

Klyro sends your AI comments to the LLM with the
[repo map](https://klyro.chat/docs/repomap.html)
