...
parent: Usage
nav_order: 760
description: Klyro can notify you when it's waiting for your input.
---

# Notifications

Klyro can notify you when it's done working and is
...
When enabled, klyro will notify you when the LLM has finished generating a response and is waiting for your input.

## OS-Specific Notifications

Klyro automatically detects your operating system and uses an appropriate notification method:
...
```bash
klyro --notifications-command "your-custom-command"
```

For example, on macOS you might use:

```bash
klyro --notifications-command "say 'Klyro is ready'"
```
...
```bash
klyro --notifications-command "apprise -b 'Klyro is ready' 'slack://your-slack-webhook-token'"
```

or Discord
```bash
klyro --notifications-command "apprise -b 'Klyro is ready' 'discord://your-discord-webhook-token'"
```

or even to your phone via Pushbullet
```bash
klyro --notifications-command "apprise -b 'Klyro is ready' 'pbul://your-pushbullet-access-token'"
```
...
```
KLYRO_NOTIFICATIONS=true
KLYRO_NOTIFICATIONS_COMMAND=your-custom-command
```
