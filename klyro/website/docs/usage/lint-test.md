...
Klyro can automatically lint and test your code
...
Klyro comes with built in linters for 
...
If there are linting errors, klyro expects the
command to print them on stdout/stderr
and return a non-zero exit code.
This is how most linters normally operate.

By default, klyro will lint any files which it edits.
...
confuse klyro into thinking there's an actual lint error that needs to be fixed.
...
Klyro will run the test command without any arguments.
If there are test errors, klyro expects the
command to print them on stdout/stderr
and return a non-zero exit code.

Klyro will try and fix any errors
if the command returns a non-zero exit code.

You can configure klyro to run your test suite
...
If you want to have klyro compile code after each edit, you
...
and optionally share the output with klyro.
This can be useful to share error messages or to show klyro
