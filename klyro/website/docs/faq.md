---
nav_order: 90
description: Frequently asked questions about klyro.
---
...
Klyro will automatically give the LLM a bunch of additional context about
...
- Use a wildcard when you launch klyro: `klyro src/*.py`
- Use a wildcard with the in-chat `/add` command: `/add src/*.py`
- Give the `/add` command a directory name and it will recursively add every file under that dir: `/add src`

## Can I use klyro in a large (mono) repo?

Klyro will work in any size repo, but is not optimized for quick
...
To get the best results from klyro you want to 
...
This will tell klyro to ignore the repo outside of the
directory you start in.

You can also create a `.klyroignore` file to tell klyro
...
you could create a `.klyroignore` file with:
...
You can use `--klyroignore <filename>` to name a specific file
...
## Can I use klyro with multiple git repos at once?

Currently klyro can only work with one repo at a time.

There are some things you can try if you need to work with
multiple interrelated repos:

- You can run klyro in repo-A where you need to make a change
and use `/read` to add some files read-only from another repo-B.
This can let klyro see key functions or docs from the other repo.
- You can run `klyro --show-repo-map > map.md` within each
repo to create repo maps.
You could then run klyro in repo-A and 
use `/read ../path/to/repo-B/map.md` to share
a high level map of the other repo.
- You can use klyro to write documentation about a repo.
Inside each repo, you could run `klyro docs.md`
and work with klyro to write some markdown docs.
Then while using klyro to edit repo-A
you can `/read ../path/to/repo-B/docs.md` to
read in those docs from the other repo.
- In repo A, ask klyro to write a small script that demonstrates
the functionality you want to use in repo B.
Then when you're using klyro in repo B, you can 
`/read` in that script.

## How do I turn on the repository map?

Depending on the LLM you are using, klyro may launch with the repo map disabled by default:
...
If you would like to force it on, you can run klyro with `--map-tokens 1024`.

## How do I include the git history in the context?

When starting a fresh klyro session, you can include recent git history in the chat context. This can be useful for providing the LLM with information about recent changes. To do this:
...
Remember, the chat history already includes recent changes made during the current session, so this tip is most useful when starting a new klyro session and you want to provide context about recent work.

You can also use klyro to review PR branches:
...
And of course you can prepare diff output outside of klyro and provide it as
a file for klyro to read:

```
$ git diff -C10 v1..v2 > v1-v2-changes.diff
$ klyro --read v1-v2-changes.diff

Klyro v0.77.2.dev+import
...
## How can I run klyro locally from source code?
...
# Install klyro in editable/development mode, 
# so it runs from the latest copy of these source files
python -m pip install -e .

# Run the local version of klyro
python -m klyro
```



## Can I change the system prompts that klyro uses?

The most convenient way to add custom instructions is to use a
[conventions file](https://klyro.chat/docs/usage/conventions.html).

But, klyro is set up to support different actual system prompts and edit formats
in a modular way. If you look in the `klyro/coders` subdirectory, you'll
...
this document about
[benchmarking GPT-3.5 and GPT-4 on code editing](https://klyro.chat/docs/benchmarks.html)
...
When experimenting with coder backends, it helps to run klyro with `--verbose --no-pretty` so you can see
all the raw information being sent to/from the LLM in the conversation.

You can also refer to the
[instructions for installing a development version of klyro](https://klyro.chat/docs/install/optional.html#install-the-development-version-of-klyro).

## What LLMs do you use to build klyro?

Klyro writes a lot of its own code, usually about 70% of the new code in each
release.
...
## How are the "klyro wrote xx% of code" stats computed?
...
## Why did klyro ignore/discard its proposed edits after it asked to add a new file to the chat?

If klyro prompts you to add a new file to the chat and you say yes,
...
So klyro re-submits the original request in this situation.

## Why does klyro sometimes stop highlighting code in its replies?

Klyro displays the markdown responses that are coming back from the LLM.
...
klyro needs to tell the LLM to use a different set of fences.
Otherwise, the LLM can't safely include your code's triple backticks
inside the code blocks that it returns with edits.
Klyro will use fences like `<source>...</source>` in this case.

A side effect of this is that the code that klyro outputs may no
...
Klyro goes to some effort to prompt the model to use the language that is configured
for your system.
...
You can explicitly set the language that klyro tells the model to use with
`--chat-language <language>`.
But the LLM may not comply.

## Can I share my klyro chat transcript?

Yes, you can now share klyro chat logs in a pretty way.

1. Copy the markdown logs you want to share from `.klyro.chat.history.md` and make a github gist. Or publish the raw markdown logs on the web any way you'd like.

   ```
   https://gist.github.com/Klyro-AI/2087ab8b64034a078c0a209440ac8be0
   ```

2. Take the gist URL and append it to:

   ```
   https://klyro.chat/share/?mdurl=
   ```

This will give you a URL like this, which shows the chat history like you'd see in a terminal:

```
https://klyro.chat/share/?mdurl=https://gist.github.com/Klyro-AI/2087ab8b64034a078c0a209440ac8be0
```

## Can I edit files myself while klyro is running?

Yes. Klyro always reads the latest copy of files from the file
system when you send each message.

While you're waiting for klyro's reply to complete, it's probably unwise to
edit files that you've added to the chat.
Your edits and klyro's edits might conflict.

## What is Klyro AI LLC?
...
Yes. You can script klyro via the command line or python. See more from here: [Scripting klyro](https://klyro.chat/docs/scripting.html)
