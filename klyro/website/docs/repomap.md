...
nav_order: 300
description: Klyro uses a map of your git repository to provide code context to LLMs.
---
...
Mapping out the repo like this provides some key benefits:
  ...
  - If it needs to see more code, the LLM can use the map to figure out which files it needs to look at. The LLM can ask to see these specific files, and klyro will offer to add them to the chat context.

## Optimizing the map

Of course, for large repositories even just the repo map might be too large
for the LLM's context window.
Klyro solves this problem by sending just the **most relevant**
...
Klyro optimizes the repo map by
...
Klyro adjusts the size of the repo map dynamically based on the state of the chat. It will usually stay within that setting's value. But it does expand the repo map
significantly at times, especially when no files have been added to the chat and klyro needs to understand the entire repo as best as possible.
...
Please check the
[repo map article on klyro's blog](https://klyro.chat/2023/10/22/repomap.html)
for more information on klyro's repository map
