...
Klyro will report an error **if a model responds** indicating that
...
For more info: https://klyro.chat/docs/token-limits.html
```

{: .note }
Klyro never *enforces* token limits, it only *reports* token limit errors
from the API provider.
The token counts that klyro reports are *estimates*.
...
It's best to only add the files that klyro will need to *edit*
...
as 4k tokens.
If you ask klyro to make a large change that affects a lot
...
Klyro has been well tested when directly connecting to 
major 
[LLM provider cloud APIs](https://klyro.chat/docs/llms.html).
For serving local models, 
[Ollama](https://klyro.chat/docs/llms/ollama.html) is known to work well with klyro.

Try using klyro without an API proxy server
