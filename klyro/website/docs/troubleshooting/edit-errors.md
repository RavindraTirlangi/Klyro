...
In these cases, klyro might say something like "Failed to apply edit to *filename*"
or other error messages.

This usually happens because the LLM is disobeying the system prompts
and trying to make edits in a format that klyro doesn't expect.
Klyro makes every effort to get the LLM
...
Klyro also sends the LLM a [map of your entire git repo](https://klyro.chat/docs/repomap.html), so other relevant code will be included automatically.
...
Most local models are just barely capable of working with klyro,
so editing errors are probably unavoidable.

## Local models: context window and quantization

Be especially careful about the
[Ollama context window](https://klyro.chat/docs/llms/ollama.html#setting-the-context-window-size)
...
because they are not capable enough to follow klyro's system prompts.

## Try the whole edit format

Run klyro with `--edit-format whole` if were using a different edit format.
...
Run klyro with `--architect` or `/chat-mode architect` to enable [architect mode](../usage/modes.md#architect-mode-and-the-editor-model).
