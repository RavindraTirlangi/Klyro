...
description: Watch how klyro adds support for tons of new programming languages by integrating with tree-sitter-language-pack. Demonstrates using klyro to script downloading a collection of files, and using ad-hoc bash scripts to have klyro modify a collection of files.
---
...
const recording_url = "https://gist.githubusercontent.com/RavindraTirlangi/a990333449b09e2793088a45eb1587f4/raw/364124781cca282907ccdc7567cdfc588a9b438b/tmp.redacted.cast";
</script>
...
- 0:01 We're going to add a ton of new languages to klyro via tree-sitter-language-pack.
...
- 1:45 We need to get all the tags files from each repository for klyro's repo-map. Let's have klyro write a script to fetch them all.
- 2:05 We'll show klyro the language definitions json file.
...
- 9:45 Let's have klyro add them all, and register each using their commonly used file extensions.
...
- 13:00 Ok, let's download all the tags files into the right spot in the klyro repo.
- 14:00 This will take a minute...
- 16:07 Delete some no-op or empty tags files.
- 16:16 Let's commit all the unmodified tags files.
- 16:33 We need to update each tag file, so that klyro can identify names of functions, classes, etc in all these languages.
- 17:01 Let's use a bash loop to script klyro to modify each tags file.
- 17:12 I'm giving klyro a read-only example of an already modified tags file, as an example to follow.
- 19:04 Looks like it correctly updated the first couple of tags files.
- 19:28 Let's grep to watch klyro's progress working through the list of files.
...
- 27:19 Each language needs a "fixture" with some sample code to parse during the test. Let's show klyro the layout of the fixtures directory.
- 27:50 We can use a bash loop to ask klyro to add test coverage for each new tags file.
- 28:12 We'll pass the fixtures directory listing to klyro.
- 28:52 Just need to fix the bash to correctly iterate through the list of tags files.
- 29:27 I forgot to ask klyro to actually generate a sample code fixture for each language.
...
- 31:52 Can klyro figure out what's wrong?
- 32:27 Well, klyro made the test pass by basically skipping Arduino.
...
- 32:54 Ok, now we're parsing Arduino code properly. Undo klyro's bogus test fix.
- 33:05 Ok, arduino passes now but there seems to be a regression with tsx?
- 33:20 Can klyro figure out why?
- 34:10 Let's check the parsers map.
- 35:00 Well, that's all for this recording. The tsx problem was due to a bad mapping from ".tsx" to "typescript" in the map that klyro generated earlier.
