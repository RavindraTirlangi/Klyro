...
const recording_url = "https://gist.githubusercontent.com/RavindraTirlangi/66b1b5aa7136147702c98afc4987c0d4/raw/4b5c7ddf7e80db1ff4dfa78fe158bc000fc42e0e/accepts-settings.cast";
</script>
...
- 0:01 Users sometimes run klyro with "reasoning" settings that aren't supported by the model they're using. This can cause LLM API calls to completely fail, with non-specific error messages from the API provider. We're going to warn users up front to prevent this.
- 0:25 Ok, let's ask klyro to add a new model setting where we can note which reasoning settings it supports. And then print a warning if the user tries to apply an unsupported setting.
...
- 4:45 Oh, we forgot to give klyro the actual file with that code! Klyro asks to see it.
- 5:00 Ok, we've confused klyro by asking it to change code it couldn't see.
...
- 14:30 Aha! I have a local model settings file for Sonnet which overrides klyro's built in settings. And we did not update it. Let's add "accepts_settings" there.
- 14:45 That was the problem, it wasn't a bug.
- 14:59 Ok, let's add test coverage for all this stuff.
- 15:09 And while klyro writes tests, let's use "git diff" to review all the changes we've made.
- 15:34 Klyro is done writing tests, let's try them.
- 15:44 One passed, one failed. Let's eyeball the passing test first.
- 16:04 And let's see if klyro can fix the failing test.
- 16:14 Klyro needs to see another file, which makes sense.
...
- 19:44 Sonnet is messing up the code editing instructions, so klyro is retrying.
...
- 21:10 And now let's have klyro update the docs to explain these changes.
- 22:32 Let's proofread and edit the updated docs.
- 24:25 And a "git diff" of all the docs changes to do a final check.
- 24:56 Let's have klyro update the project's HISTORY file.
