...
[![benchmark results](/assets/benchmarks-1106.svg)](https://klyro.chat/assets/benchmarks-1106.svg)

[![benchmark results](/assets/benchmarks-speed-1106.svg)](https://klyro.chat/assets/benchmarks-speed-1106.svg)
...
To do this, klyro needs to be able to reliably recognize when GPT wants to edit
your source code,
...
Klyro relies on a
[code editing benchmark](https://klyro.chat/docs/benchmarks.html)
to quantitatively evaluate
performance
whenever one of these things changes.
For example,
whenever I change klyro's prompting or the backend which drives LLM conversations,
I run the benchmark to make sure these changes produce improvements (not regressions).

The benchmark uses klyro to try and complete
...
The benchmark gives klyro two tries to complete the task:

1. On the first try, klyro gives GPT the stub code file to edit and the natural language instructions that describe the problem. This reflects how you code with klyro. You add your source code files to the chat and ask for changes, which are automatically applied.
2. If the test suite fails after the first try, klyro gives GPT the test error output and asks it to fix the code. Klyro supports this sort of interaction using a command like `/run pytest` to run and share pytest results in the chat with GPT. You can `/run` whatever tests/linters/etc make sense for your language/framework/situation.
...
This is the edit format that klyro uses by default with gpt-4.
...
that use the klyro benchmarking suite to assess and compare the code
editing capabilities of OpenAI's GPT models.
You can review the other reports
for additional information:

- [GPT code editing benchmarks](https://klyro.chat/docs/benchmarks.html) evaluates the March and June versions of GPT-3.5 and GPT-4.
- [Code editing speed benchmarks for OpenAI's "1106" models](https://klyro.chat/2023/11/06/benchmarks-speed-1106.html) compares the performance of the new GPT models.
