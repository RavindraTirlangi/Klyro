...
Klyro is configured to work with most popular reasoning models out of the box. 
You can use them like this:

```bash
# Sonnet uses a thinking token budget
klyro --model sonnet --thinking-tokens 8k

# o3-mini uses low/medium/high reasoning effort
klyro --model o3-mini --reasoning-effort high

# R1 doesn't have configurable thinking/reasoning
klyro --model r1
```

Inside the klyro chat, you can use `/thinking-tokens 4k` or `/reasoning-effort low` to change
...
if you're configuring klyro to work with a lesser known reasoning model or one served
via an unusual provider.

## Reasoning settings

Different models support different reasoning settings. Klyro provides several ways to control reasoning behavior:
...
Not all models support these two settings. Klyro uses the 
...
If you try to use a setting that a model doesn't explicitly support, Klyro will warn you:
...
This configuration:
1. Tells Klyro that the model accepts the `reasoning_effort` setting
2. Indicates the model does NOT accept `thinking_tokens` (since it's not listed)
3. Causes Klyro to ignore any `--thinking-tokens` value passed for this model
...
1. Force Klyro to apply all settings passed via command line
...
`<think>...</think>` tags, so klyro's settings
...
Klyro will display the thinking/reasoning output, 
but it won't be used for file editing instructions, added to the chat history, etc.
Klyro will rely on the non-thinking output for instructions on how to make code changes, etc.
...
Klyro is configured to work properly with popular models
...
Include settings for your new provider in `.klyro.model.settings.yml` file
...
Note that klyro already has these settings pre-configured, but they
