...
In most cases, you can safely ignore klyro's warning about unknown context
window size and model costs.

{: .note }
Klyro never *enforces* token limits, it only *reports* token limit errors
from the API provider.
You probably don't need to
configure klyro with the proper token limits
for unusual models.

But, you can register context window limits and costs for models that aren't known
to klyro. Create a `.klyro.model.metadata.json` file in one of these locations:

- Your home directory.
- The root if your git repo.
- The current directory where you launch klyro.
...
in the `.klyro.model.metadata.json` file.
...
Klyro has a number of settings that control how it works with
different models.
...
a model that klyro doesn't know about.


### Configuration file locations

You can override or add settings for any model by creating a `.klyro.model.settings.yml` file in one of these locations:
...
You can use the special model name `klyro/extra_params` to define 
...
`klyro/extra_params` settings taking precedence for any direct conflicts.
...
<!--[[[cog
from klyro.models import get_model_settings_as_yaml
cog.out("```yaml\n")
