...
Klyro can connect to models provided by Amazon Bedrock.
To configure Klyro to use the Amazon Bedrock API, you need to set up your AWS credentials.
...
Once your AWS credentials are set up, you can run Klyro with the `--model` command line switch, specifying the Bedrock model you want to use:

```bash
# Change directory into your codebase
cd /to/your/project

klyro --model bedrock/anthropic.claude-3-5-sonnet-20240620-v1:0
```

Sometimes it seems to help if you prefix the model name with "us.":

```bash
klyro --model bedrock/us.anthropic.claude-3-5-sonnet-20240620-v1:0
```
...
```bash
klyro --list-models bedrock/
```

Make sure you have access to these models in your AWS account before attempting to use them with Klyro.
