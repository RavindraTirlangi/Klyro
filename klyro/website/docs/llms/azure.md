...
Klyro can connect to the OpenAI models on Azure.
...
Start working with klyro and Azure on your codebase:

```bash
# Change directory into your codebase
cd /to/your/project

klyro --model azure/<your_model_deployment_name>

# List models available from Azure
klyro --list-models azure/
```

Note that klyro will also use environment variables
like `AZURE_OPENAI_API_xxx`.

The `klyro --list-models azure/` command will list all models that klyro supports through Azure, not the models that are available for the provided endpoint.
