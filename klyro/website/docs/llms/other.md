...
Klyro uses the [litellm](https://docs.litellm.ai/docs/providers) package
to connect to hundreds of other models.
You can use `klyro --model <model-name>` to use any supported model.

To explore the list of supported models you can run `klyro --list-models <model-name>`
with a partial model name.
If the supplied name is not an exact match for a known model, klyro will
return a list of possible matching models.
For example:

```
$ klyro --list-models turbo
...
when working with models that klyro is not familiar with.

## LiteLLM

Klyro uses the LiteLLM package to connect to LLM providers.
