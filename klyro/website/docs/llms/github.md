...
Klyro can connect to GitHub Copilot’s LLMs because Copilot exposes a standard **OpenAI-style**
endpoint at:
...
Each returned ID can be used with klyro by **prefixing it with `openai/`**:

```bash
klyro --model openai/gpt-4o
# or
klyro --model openai/claude-3.7-sonnet-thought
```
...
## Optional config file (`~/.klyro.conf.yml`)
...
* Calls made through klyro are billed through your Copilot subscription  
  (klyro will still print *estimated* costs).
* The Copilot docs explicitly allow third-party “agents” that hit this API – klyro is playing by
  the rules.
* Klyro talks directly to the REST endpoint—no web-UI scraping or browser automation.
