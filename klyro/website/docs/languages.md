...
parent: More info
nav_order: 200
description: Klyro supports pretty much all popular coding languages.
---
# Supported languages

Klyro should work well with most popular coding languages.
...
Klyro has specific support for linting many languages.
By default, klyro runs the built in linter any time a file is edited.
If it finds syntax errors, klyro will offer to fix them for you.
This helps catch small code issues and quickly fix them.

Klyro also does code analysis to help
the LLM navigate larger code bases by producing
a [repository map](https://klyro.chat/docs/repomap.html).
Klyro can currently produce repository maps for many popular
mainstream languages, listed below.


## How to add support for another language

Klyro should work quite well for other languages, even those
without repo map or linter support.
You should really try coding with klyro before
assuming it needs better support for your language.

That said, if klyro already has support for linting your language,
...
language parsers.
This makes it easy for users to install klyro in many diverse environments.
You probably need to work with that project to get your language
supported, which will easily allow klyro to lint that language.
For repo-map support, you will also need to find or create a `tags.scm` file.

<!--[[[cog
from klyro.repomap import get_supported_languages_md
