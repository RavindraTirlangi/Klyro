...
---
{% if page.date %}
<p class="post-date">{{ page.date | date: "%B %d, %Y" }}, by Ravindra Tirlangi
</p>
{% endif %}
...
Klyro now asks GPT-4 Turbo to use
...
Klyro's new "laziness" benchmark suite 
...
- **GPT-4 Turbo only scored 20% as a baseline** using klyro's existing "SEARCH/REPLACE block" edit format. It outputs "lazy comments" on 12 of the tasks.
- **Klyro's new unified diff edit format raised the score to 61%**. Using this format reduced laziness by 3X, with GPT-4 Turbo only using lazy comments on 4 of the tasks.
...
- **The June GPT-4's baseline was 26%** using klyro's existing "SEARCH/REPLACE block" edit format.
- **Klyro's new unified diff edit format raised June GPT-4's score to 59%**. 
...
Klyro's new unified diff editing format
...
numerous variations on klyro's existing editing formats,
...
klyro's new editing format and refactoring benchmark.
...
The design and implementation of klyro's new unified diff editing format
...
Klyro's [previous benchmark results](https://klyro.chat/docs/benchmarks.html) made
...
Klyro tells GPT not to include line numbers,
...
Klyro's system prompt encourages
...
When a patch fails, klyro needs to ask GPT for a corrected version of the diff.
...
Klyro tries to be very flexible when applying diffs,
in order to handle defects.
If a hunk doesn't apply cleanly, klyro uses a number of strategies:
...
**Experiments where flexible patching is disabled show a 9X increase in editing errors** on klyro's original Exercism benchmark.

## Refactoring benchmark

Klyro has long used a
[benchmark suite based on 133 Exercism python exercises](https://klyro.chat/2023/07/02/benchmarks.html).
...
[9 popular open source python repositories](https://github.com/Klyro-AI/refactor-benchmark)
...
A [simple python AST scanning script](https://github.com/RavindraTirlangi/Klyro/blob/main/benchmark/refactor_tools.py)
...
[benchmark suite that provokes, detects and quantifies GPT coding laziness](https://github.com/Klyro-AI/refactor-benchmark).
...
klyro's new unified diff format seems
...
when originally building klyro.
...
incorporated into klyro provide
...
klyro's simple, high level style of unified diffs.
