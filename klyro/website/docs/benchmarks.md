...
[![benchmark results](/assets/benchmarks.svg)](https://klyro.chat/assets/benchmarks.svg)

Klyro is an open source command line chat tool that lets you work with GPT to edit
code in your local git repo.
To do this, klyro needs to be able to reliably recognize when GPT wants to edit local files,
...
Klyro currently asks GPT to use simple text based "edit formats", but
...
and *format those code edits* so that klyro can save the
...
Using more complex output formats with GPT seems to cause two issues:
  ...
  - It reduces GPT's adherence to the output format, making it more challenging for tools like klyro to accurately identify and apply the edits GPT is attempting to make.

I was expecting to start using function call based edits in klyro for both GPT-3.5 and GPT-4.
...
To start each exercise, klyro sends GPT
...
Klyro updates the implementation file based on GPT's reply and runs
the unit tests. If all tests pass, the exercise is considered
complete. If some tests fail, klyro sends GPT a second message with
the test error output. It only sends the first 50 lines of test errors
to try and avoid exceeding the context window of the smaller models. Klyro
...
In summary, passing an exercise means GPT was able to:
  ...
  - Correctly package all of the code edits into the edit format so that klyro can process and save it to the implementation file.
...
The
[whole](https://github.com/RavindraTirlangi/Klyro/blob/main/klyro/coders/wholefile_prompts.py)
...
The [diff](https://github.com/RavindraTirlangi/Klyro/blob/main/klyro/coders/editblock_prompts.py)
...
The [whole-func](https://github.com/RavindraTirlangi/Klyro/blob/main/klyro/coders/wholefile_func_coder.py)
...
The
[diff-func](https://github.com/RavindraTirlangi/Klyro/blob/main/klyro/coders/editblock_func_coder.py)
...
Based on these benchmark results, klyro will continue to use
...
Klyro displays a progress bar and
