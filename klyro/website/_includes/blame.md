...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var blameData = {
        labels: labels,
        datasets: [{
            label: 'Klyro\'s percent of new code by release',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.klyro_percentage }}, lines: {{ row.klyro_total }}, end_date: '{{ row.end_date }}' },{% endfor %}],
    ...
    var linesData = {
        ...
        datasets: [{
            label: 'Klyro',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.klyro_total }} },{% endfor %}],
            backgroundColor: 'rgba(54, 162, 235, 0.8)',
            borderColor: 'rgba(54, 162, 235, 1)',
            borderWidth: 1
        },
        {
            label: 'Human',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.total_lines | minus: row.klyro_total }} },{% endfor %}],
    ...
    var blameChart = new Chart(blameCtx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            var label = 'Klyro\'s contribution';
                ...
                title: {
                    display: true,
                    text: 'Percent of new code written by klyro, by release',
                    font: {
                        size: 16
                    }
                }
